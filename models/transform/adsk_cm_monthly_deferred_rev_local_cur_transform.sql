{{ config(
    alias='adsk_cm_monthly_deferred_local_cur_rev'
) }}
...
*/
    ...
    FROM            (SELECT
                    ...
                    FROM {{ source('tenrox_private', 'tfcalperiod') }} tfcalperiod
                        INNER JOIN {{ source('tenrox_private', 'tproject') }} tproject
    ...
    FULL OUTER JOIN {{ ref('adsk_cm_monthly_expect_labor_rev_local_cur_transform') }} AS monthly_expect_labor_rev
    ...
    FULL OUTER JOIN {{ ref('adsk_cm_monthly_chrg_rev_local_cur_transform') }} AS monthly_chrg_rev 
    ...
    FULL OUTER JOIN (SELECT
                      ...
                      , monthrecognized
                      , recognizedrevenue
                    FROM {{ ref('adsk_cm_monthly_rec_rev_local_cur_transform') }}
