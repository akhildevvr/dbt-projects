...
LEFT JOIN
    (
        ...
        FROM {{ source('tenrox_private', 'twfinstanceobject') }} twfinstanceobject
        INNER JOIN
            {{ source('tenrox_private', 'twfworkflowversion') }} twfworkflowversion
            ON twfworkflowversion.uniqueid = twfinstanceobject.workflowversionid
        INNER JOIN
            {{ source('tenrox_private', 'twfworkflow') }} twfworkflow
            ON twfworkflow.uniqueid = twfworkflowversion.workflowid
        INNER JOIN
            {{ source('tenrox_private', 'twfinstanceactivity') }} AS wfinstact
            ON wfinstact.instanceguid = twfinstanceobject.instanceguid
        INNER JOIN
            {{ source('tenrox_private', 'twfworkflowactivitydesc') }} AS wfactivitydesc
...
LEFT JOIN {{ source('tenrox_private', 'tportfolio') }} tportfolio
    ON tportfolio.uniqueid = tproject.portfolioid
LEFT JOIN {{ source('tenrox_private', 'tuser') }} pm 
    ON tproject.managerid = pm.uniqueid
LEFT JOIN {{ source('tenrox_private', 'tuser') }} pmalt 
    ON tproject.alternatemgrid = pmalt.uniqueid
LEFT JOIN {{ source('tenrox_private', 'tuser') }} portman 
...
LEFT JOIN {{ source('tenrox_private', 'tcurrency') }} tcurrency 
