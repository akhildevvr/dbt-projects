{{ config(
    alias='adsk_cm_forecast_chrg_rev_v02'
) }}
...
SELECT 
    ...
    INNER JOIN
        {{ source('tenrox_private', 'trvfcbltmpl') }} trvfcbltmpl
        ON trvfcbltmpl.baselineid = trvfcbaseline.uniqueid
    INNER JOIN
        {{ source('tenrox_private', 'trvfcblsec') }} trvfcblsec 
    ...
    LEFT JOIN
        {{ source('tenrox_private', 'tclientinvoice') }} tclientinvoice
