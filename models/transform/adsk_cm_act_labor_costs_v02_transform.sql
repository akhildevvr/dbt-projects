{{ config(
    alias='adsk_cm_act_labor_costs_v02'
) }}
...
FROM {{ source('tenrox_private', 'ttimeentryrate') }} ttimeentryrate
LEFT OUTER JOIN {{ ref('adsk_month_q_ranges_v02_transform') }} AS ranges  -- put back to original place, join before TTIMESHEETENTRIES
JOIN {{ source('tenrox_private', 'ttimesheetentries') }} ttimesheetentries 
     ON ttimeentryrate.timeentryuid = ttimesheetentries.timeentryuid
JOIN {{ source('tenrox_private', 'ttask') }} ttask ON ttask.uniqueid = ttimesheetentries.taskuid
JOIN {{ source('tenrox_private', 'tproject') }} tproject ON tproject.uniqueid = ttask.projectid
JOIN {{ source('tenrox_private', 'tclientinvoice') }} tclientinvoice ON tclientinvoice.clientid = tproject.clientid
LEFT OUTER JOIN
    (
