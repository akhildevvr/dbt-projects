{{ config(
    alias='adsk_cm_labor_rev_local_cur_v02'
) }}
...
LEFT JOIN {{ ref('adsk_cm_project_budget_local_cur_transform') }} as projbudget
             ON projbudget.projectid = tproject.uniqueid
LEFT JOIN {{ ref('adsk_cm_labor_hrs_v02_transform') }} as laborhrs
             ON laborhrs.projectid = tproject.uniqueid
LEFT JOIN {{ ref('adsk_cm_deferred_rev_local_cur_transform') }} as deferred_rev
