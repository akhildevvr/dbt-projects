{{ config(
    alias='adsk_cm_forecast_chrg_rev_local_cur_v02'
) }}
...
SELECT 
    ...
    FROM {{ source('tenrox_private', 'trvfcbaseline') }} trvfcbaseline
    LEFT OUTER JOIN {{ source('tenrox_private', 'trvfcbaselinebudget') }} trvfcbaselinebudget
        ON trvfcbaselinebudget.baselineuid = trvfcbaseline.uniqueid 
    INNER JOIN {{ source('tenrox_private', 'trvfcbltmpl') }} trvfcbltmpl 
        ON trvfcbltmpl.baselineid = trvfcbaseline.uniqueid 
    INNER JOIN {{ source('tenrox_private', 'trvfcblsec') }} trvfcblsec 
        ON trvfcblsec.bltmplid = trvfcbltmpl.uniqueid 
    INNER JOIN {{ source('tenrox_private', 'trvfcblseclabel') }} trvfcblseclabel 
        ON trvfcblseclabel.blsecid = trvfcblsec.uniqueid 
        AND trvfcblseclabel.language = 0 
    INNER JOIN {{ source('tenrox_private', 'trvfcblcat') }} trvfcblcat 
        ON trvfcblcat.blsecid = trvfcblsec.uniqueid 
    INNER JOIN {{ source('tenrox_private', 'trvfcblcatlabel') }} trvfcblcatlabel 
        ON trvfcblcatlabel.blcatid = trvfcblcat.uniqueid 
        AND trvfcblcatlabel.language = 0 
    INNER JOIN {{ source('tenrox_private', 'trvfcblitem') }} trvfcblitem 
        ON trvfcblitem.blcatid = trvfcblcat.uniqueid 
    INNER JOIN {{ source('tenrox_private', 'trvfcblitemdata') }} trvfcblitemdata 
        ON trvfcblitemdata.blitemid = trvfcblitem.uniqueid 
    INNER JOIN {{ source('tenrox_private', 'tfcalperiod') }} tfcalperiod 
    ...
    RIGHT OUTER JOIN {{ source('tenrox_private', 'tproject') }} tproject
