...
FROM {{ source('tenrox_private', 'tcustfld') }}  AS cf
INNER JOIN {{ source('tenrox_private', 'tcustlst') }}  AS cl
    ON cf.uniqueid = cl.custfldid
INNER JOIN {{ source('tenrox_private', 'tcustlstdesc') }}  AS cld 
