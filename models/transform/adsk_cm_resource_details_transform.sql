{{ config(
    alias='adsk_cm_resource_details'
) }}
...
WITH fxrate AS (
    SELECT * FROM {{ ref('fcurrqexchrate_transform') }}
    ...
    LEFT OUTER JOIN {{ source('tenrox_private', 'ttitle') }} ttitle 
        ON ttitle.uniqueid = tuser.titleid
    LEFT OUTER JOIN {{ source('tenrox_private', 'tmapdata') }} mapdata_secgroup
        ON mapdata_secgroup.fieldkey = tuser.security
        AND mapdata_secgroup.language = 0
        AND mapdata_secgroup.tablename = 'SECURITY'
    LEFT OUTER JOIN {{ source('tenrox_private', 'tmapdata') }} mapdata_usertype
        ON mapdata_usertype.fieldkey = tuser.usertype
        AND mapdata_usertype.language = 0
        AND mapdata_usertype.tablename = 'USERTYPE'
    LEFT OUTER JOIN {{ source('tenrox_private', 'tgroup') }} group_app 
        ON group_app.uniqueid = tuser.groupid
    LEFT OUTER JOIN {{ source('tenrox_private', 'tgroup') }} group_funct 
        ON group_funct.uniqueid = tuser.functionalgroupid
    LEFT OUTER JOIN {{ source('tenrox_private', 'tgroup') }} group_user 
        ON group_user.uniqueid = tuser.resgroupid
    LEFT OUTER JOIN {{ source('tenrox_private', 'tresourcetypehist') }} tresourcetypehist
    ...
    LEFT OUTER JOIN {{ source('tenrox_private', 'tresourcetype') }} tresourcetype 
    ...
    SELECT 
          ...
          , 3 AS sqlversion_resource_details
    FROM {{  source('tenrox_private', 'tplanningrole') }} tplanningrole