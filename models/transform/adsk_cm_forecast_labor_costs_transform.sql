{{ config(
    alias='adsk_cm_forecast_labor_costs'
) }}
...
*/
        ...
        INNER JOIN {{ source('tenrox_private', 'tclientinvoice') }} tclientinvoice
            ON tclientinvoice.clientid = tproject.clientid 
        INNER JOIN {{ source('tenrox_private', 'trplnbooking') }} trplnbooking 
            ON tproject.uniqueid = trplnbooking.projectid
        INNER JOIN {{ source('tenrox_private', 'trplnbookingdetails') }} trplnbookingdetails 
            ON trplnbookingdetails.bookingid = trplnbooking.uniqueid 
        INNER JOIN {{ source('tenrox_private', 'trplnbookingattributes') }} trplnbookingattributes 
            ON trplnbookingattributes.bookingid = trplnbooking.uniqueid
        INNER JOIN {{ source('tenrox_private', 'tprojectteamresource') }} tprojectteamresource 
