...
FROM {{ source('tenrox_private', 'tprojectcustfld') }} AS a
LEFT JOIN {{ source('tenrox_private', 'tsite') }} AS tsite3
    ON a.adsk_geo = tsite3.uniqueid
LEFT JOIN {{ source('tenrox_private', 'tcustlst') }} AS lst_11 
    ON a.adsk_projaccountant = lst_11.uniqueid
LEFT JOIN {{ source('tenrox_private', 'tcustlstdesc') }} AS lstdesc_11 
    ON lst_11.uniqueid = lstdesc_11.custlstid AND lstdesc_11.language = 0
LEFT JOIN {{ source('tenrox_private', 'tcustlst') }} AS lst_16 
    ON a.adsk_masteragreement_projecttype = lst_16.uniqueid
LEFT JOIN {{ source('tenrox_private', 'tcustlstdesc') }} AS lstdesc_16 
    ON lst_16.uniqueid = lstdesc_16.custlstid AND lstdesc_16.language = 0
LEFT JOIN {{ source('tenrox_private', 'tcustlst') }} AS lst_20 
    ON a.adsk_revrectreatment = lst_20.uniqueid 
LEFT JOIN {{ source('tenrox_private', 'tcustlstdesc') }} AS lstdesc_20 
    ON lst_20.uniqueid = lstdesc_20.custlstid AND lstdesc_20.language = 0
LEFT JOIN {{ source('tenrox_private', 'tcustlst') }} AS lst_36 
    ON a.adsk_accountingcontracttype = lst_36.uniqueid 
LEFT JOIN {{ source('tenrox_private', 'tcustlstdesc') }} AS lstdesc_36 
