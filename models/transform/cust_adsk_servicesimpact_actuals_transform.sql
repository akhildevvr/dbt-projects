{{ config(
    alias='cust_adsk_servicesimpact_actuals'
) }}
...
WITH tmp_cust_adsk_servicesimpact_actuals AS (
        ...
        FROM {{ source('tenrox_private', 'ttimesheetentries') }} AS tse
        INNER JOIN {{ source('tenrox_private', 'ttask') }} AS t ON tse.taskuid = t.uniqueid
        INNER JOIN {{ source('tenrox_private', 'tuserplanningrole') }} AS upr 
            ON upr.userid = tse.useruid AND upr.isprimaryrole = 1
        INNER JOIN {{ source('tenrox_private', 'tplanningrole') }} AS pr 
            ON pr.uniqueid = upr.planningroleid
        WHERE tse.entrydate >= DATE '1990-02-01'
            AND tse.approved = 1
            AND t.projectid NOT IN (SELECT uniqueid 
                                    FROM {{ source('tenrox_private', 'tproject') }} 
                                    WHERE releasealias = 'A-ADMIN-00001')
            AND t.projectid NOT IN (
                SELECT projectid
                FROM {{ ref('adsk_cm_project_details_transform') }}
                WHERE
                    projectstate IN (
                        'SCO-Funnel',
                        'Funnel',
