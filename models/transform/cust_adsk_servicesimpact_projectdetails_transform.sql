{{ config(
    alias='cust_adsk_servicesimpact_projectdetails'
) }}
...
WITH tmp_cust_adsk_servicesimpact_projectdetails AS (
    SELECT 
      ...
      INNER JOIN {{ ref('tprojectcustfld_view_transform') }} AS udfs
          ON udfs.projectid = project_details.projectid 
      INNER JOIN {{ source('tenrox_private', 'tclient') }} tclient
          ON tclient.uniqueid = project_details.clientid 
      LEFT OUTER JOIN {{ ref('adsk_cm_project_budget_transform') }} AS project_budget
      ...
      LEFT OUTER JOIN (SELECT
                      ...
                      JOIN {{ source('tenrox_private', 'tproject') }} tproject 
