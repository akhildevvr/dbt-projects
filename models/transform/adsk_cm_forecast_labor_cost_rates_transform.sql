{{ config(
    alias='adsk_cm_forecast_labor_cost_rates'
) }}
...
SELECT 
    ...
    FROM {{ source('tenrox_private', 'tprojectteamresource') }} AS ptr
    LEFT JOIN
        {{ source('tenrox_private', 'tuser') }} AS u
        ON u.uniqueid = ptr.resourceid AND ptr.isrole = 0 
    LEFT JOIN {{ source('tenrox_private', 'tplanningrole') }} AS plr 
     ...
     LEFT JOIN (SELECT /* Project-to-User has priority */
                        ...
                        FROM {{ source('tenrox_private', 'trateassocobjectlink') }} AS rao
                        JOIN {{ source('tenrox_private', 'traterule') }} AS rr
                            ON (rao.rateruleid = rr.uniqueid) 
                        JOIN {{ source('tenrox_private', 'trateruleentry') }} AS re 
     ...
     LEFT JOIN (SELECT /* If no specified project rate for this user then use User rate with RATEMODE 1016 */
                        ...
                        FROM {{ source('tenrox_private', 'trateassocobjectlink') }} AS rao
                        JOIN
                            {{ source('tenrox_private', 'traterule') }} AS rr
                            ON (rao.rateruleid = rr.uniqueid)
                        JOIN
                            {{ source('tenrox_private', 'trateruleentry') }} AS re
                            ON (re.rateruleid = rr.uniqueid)
                        WHERE
                            rr.type = 0  -- COST
                            AND rr.processing = 0
