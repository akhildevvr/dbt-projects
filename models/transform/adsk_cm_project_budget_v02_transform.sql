{{ config(
    alias='adsk_cm_project_budget_v02'
) }}
...
*/
    ...
    LEFT OUTER JOIN
        {{ source('tenrox_private', 'tbudgetdetail') }} tbudgetdetail ON tbudgetdetail.objectid = tproject.uniqueid
    LEFT OUTER JOIN
    ...
    LEFT OUTER JOIN {{ source('tenrox_private', 'tcharge') }} tcharge ON tcharge.uniqueid = tbudgetdetailentry.objectid
    LEFT OUTER JOIN {{ source('tenrox_private', 'tclient') }} tclient ON tclient.uniqueid = tproject.clientid
    LEFT OUTER JOIN
        {{ source('tenrox_private', 'tclientinvoice') }} tclientinvoice ON tclientinvoice.clientid = tclient.uniqueid
    LEFT OUTER JOIN
        {{ ref('tprojectcustfld_view_transform') }} tprojectcustfld_view
