...
*/
   SELECT 
    ...
    LEFT JOIN {{ source('tenrox_private', 'tbudgetdetail') }} tbudgetdetail ON tbudgetdetail.objectid = tproject.uniqueid
    LEFT JOIN
        {{ source('tenrox_private', 'tbudgetdetaillist') }} tbudgetdetaillist
        ON tbudgetdetail.objecttype = 2
        AND tbudgetdetail.uniqueid = tbudgetdetaillist.budgetdetailedid
    LEFT JOIN
        {{ source('tenrox_private', 'tbudgetdetailentry') }} tbudgetdetailentry
        ON tbudgetdetaillist.uniqueid = tbudgetdetailentry.budgetdetailedlistid
    LEFT JOIN {{ source('tenrox_private', 'tcharge') }} tcharge ON tcharge.uniqueid = tbudgetdetailentry.objectid
    LEFT JOIN {{ source('tenrox_private', 'tclient') }} tclient ON tclient.uniqueid = tproject.clientid
    LEFT JOIN {{ source('tenrox_private', 'tclientinvoice') }} tclientinvoice ON tclientinvoice.clientid = tclient.uniqueid
    LEFT JOIN (SELECT currencyid FROM {{ source('tenrox_private', 'tsysdefs') }} tsysdefs WHERE uniqueid = 1) basecurrency
    LEFT OUTER JOIN
        (
    ...
    LEFT OUTER JOIN
        {{ ref('fcurrqexchrate_transform') }} AS fxrate
