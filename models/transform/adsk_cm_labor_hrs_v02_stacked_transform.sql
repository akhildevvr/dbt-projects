{{ config(
    alias='adsk_cm_labor_hrs_v02_stacked'
) }}
...
(
  ...
  *
  FROM
  {{ source('tenrox_private','ttimesheetentries')}}
),
task AS
(
  SELECT
...
(
  ...
  *
  FROM
  {{ source('tenrox_private','trplnbooking')}}
...
(
  ...
  *
  FROM
  {{ source('tenrox_private','trplnbookingdetails')}}
...
(
  ...
  *
  FROM
  {{ source('tenrox_private','trplnbookingattributes')}}
...
(
  ...
  *
  FROM
  {{ source('tenrox_private','tprojectteamresource')}}
...
(
  ...
  *
  FROM
  {{ ref('adsk_cm_labor_hrs_v02_transform')}}
),
