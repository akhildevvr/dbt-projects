{{ config(
    alias='cust_adsk_marginvariance'
) }}
...
FROM {{ ref('adsk_cm_project_details_transform') }} AS project_details
LEFT OUTER JOIN {{ ref('tprojectcustfld_view_transform') }} ON tprojectcustfld_view.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_const_curr_transform') }} AS const_curr ON const_curr.currencycode = project_details.projectcurrency
LEFT OUTER JOIN {{ ref('adsk_cm_project_budget_transform') }} AS project_budget ON project_budget.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_act_charge_costs_v02_transform') }} AS act_charge_costs ON act_charge_costs.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_act_labor_costs_v02_transform') }} AS act_labor_costs ON act_labor_costs.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_forecast_chrg_rev_v02_transform') }} AS forecast_chrg_rev ON forecast_chrg_rev.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_forecast_chrg_cost_transform') }} AS forecast_chrg_cost ON forecast_chrg_cost.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_forecast_labor_costs_transform') }} AS forecast_labor_costs ON forecast_labor_costs.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_labor_hrs_v02_transform') }} AS labor_hrs ON labor_hrs.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_labor_rev_v02_transform') }} AS labor_rev ON labor_rev.projectid = project_details.projectid
LEFT OUTER JOIN {{ ref('adsk_cm_rec_chrg_rev_v02_transform') }} AS rec_chrg_rev ON rec_chrg_rev.projectid = project_details.projectid
LEFT OUTER JOIN {{ source('tenrox_private', 'tuser') }} tuser ON tuser.uniqueid = project_details.projectmanagerid
LEFT OUTER JOIN {{ source('tenrox_private', 'tsite') }} pm_site_active ON pm_site_active.uniqueid = tuser.activesiteid
LEFT OUTER JOIN
 (  
  SELECT
    ...
    JOIN {{ source('tenrox_private', 'ttask') }} ttask ON ttimesheetentries.taskuid = ttask.uniqueid 
    JOIN {{ source('tenrox_private', 'tproject') }} tproject ON tproject.uniqueid = ttask.projectid
