{{ config(
    alias='project_forecast_stacked'
) }}
...
(
    ...
    *
    FROM
    {{ source('TENROX_PRIVATE', 'TWORKFLOWMAP') }}
...
(
    ...
    *
    FROM
    {{ source('tenrox_transform','adsk_cm_project_details') }} 
),
...
(
    ...
    *
    FROM
    {{ source('tenrox_transform','tprojectcustfld_view') }}
),
...
(
    ...
    *
    FROM
    {{ source('tenrox_transform','adsk_cm_project_budget_local_cur') }}
),
...
(
    ...
    *
    FROM
    {{ source('tenrox_transform','adsk_cm_labor_hrs_v02_stacked') }}
),
 ...
 *
 FROM
 {{ source('tenrox_transform','adsk_cm_monthly_deferred_local_cur_rev_stacked')}}
 ...
 *
 FROM
 {{ source('tenrox_transform','adsk_cm_labor_rev_local_cur_v02_stacked')}}
),
...
(
  ...
  *
 FROM
 {{ source('tenrox_transform','adsk_cm_forecast_chrg_rev_local_cur_v02_stacked')}}
),
...
(
  ...
  *
 FROM
 {{ source('tenrox_transform','adsk_cm_rec_chrg_rev_local_cur_v02_stacked')}}
...
(
  ...
  *
 FROM
 {{ source('tenrox_transform','adsk_cm_monthly_chrg_rev_local_cur_stacked')}}
...
(
  ...
  *
 FROM
 {{ source('tenrox_transform','adsk_cm_rec_rev_v02_stacked')}}
),
...
(
    ...
    *
    FROM
    {{ source('tenrox_transform','adsk_month_q_ranges_v02')}}
),
...
fcurrqexchrate AS (
	...
	*
	FROM
	{{ source('tenrox_transform','fcurrqexchrate')}}
),
