  ...
  create or replace   view EIO_INGEST.ENGAGEMENT_TRANSFORM.as_is_tenrox_parent_projects
...
margin_variance as (
 ...
 SELECT
    ...
    , CASE
        ...
        WHEN lower(mv.projecttype) = 'is master' THEN CONCAT(pd.master_agreement_nm, '-IS')
        WHEN lower(mv.projecttype) = 'as master' THEN CONCAT(pd.master_agreement_nm, '-AS')
...
WHERE
    lower(mv.projecttype) in ('is master', 'as master', 'is parent-child',
                                    'as parent-child')
